...
class WhitelistTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->kernel = m::mock('Symfony\Component\HttpKernel\HttpKernelInterface', function ($mock) use ($successfulResponse) {
            $mock->shouldReceive('handle')->once()->andReturn($successfulResponse);
        });
        $this->stackedApp = new Whitelist($this->kernel, $this->getListedIpAddresses());
     ...
     * @return void
     */
    public function itReturns200ForListedIpAddresses($clientIp)
    {
     ...
     * @return void
     */
    public function itReturns403ForNonListedIpAddresses($clientIp)
    {
        $request = m::mock('Symfony\Component\HttpFoundation\Request', function ($mock) use ($clientIp) {
            $mock->shouldReceive('getClientIp')->once()->andReturn($clientIp);
        });
